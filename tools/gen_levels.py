...
SPR_PALS = [
    ...
    [(0x16, 0x27, 0x37), (0x0F, 0x16, 0x30), (0x0F, 0x00, 0x10), (0x0F, 0x27, 0x37)],
    # the Left Foot is the same shape as the player in a colder colour:
    # it should read as the other one, not as a second protagonist
    [(0x16, 0x27, 0x37), (0x02, 0x12, 0x30), (0x0F, 0x0C, 0x2C), (0x0F, 0x28, 0x38)],
]
...
SOLID_CLASSES = (1, 4, 6, 7)            # solid, breakable, ice, mud
STAND_CLASSES = SOLID_CLASSES + (2, 5)  # ... platforms, and water to swim in

JUMP_UP = 2                             # metatiles a full jump clears
JUMP_ACROSS = 3                         # columns a running jump crosses


def check_traversal(mapdata, cols, collision, start, boss_col, name):
    """Can the foot actually walk from the start to the boss?

    Surfaces are every standable tile with headroom above it; the search
    then walks and jumps between them the way the player can -- up two
    metatiles, across three columns, down any distance.  This is a
    build-time check because the bug it exists to catch, a boss arena
    walled in on all four sides, looked perfectly normal in a screenshot.
    """
    def cls(col, row):
        if col < 0 or col >= cols or row < 0 or row >= CHH:
            return 1 if row >= CHH else 0
        return collision[mapdata[col * MAP_STRIDE + row]]

    surfaces = [[] for _ in range(cols)]
    for col in range(cols):
        for row in range(CHH):
            if cls(col, row) in STAND_CLASSES and cls(col, row - 1) not in SOLID_CLASSES:
                surfaces[col].append(row)

    def landing(col, from_row):
        """The surface the foot ends up on stepping into `col`."""
        best = None
        for row in surfaces[col]:
            if row >= from_row - JUMP_UP:
    ...
    seen = set()
    stack = []
    for row in surfaces[start[0]] or []:
        stack.append((start[0], row))
    if not stack:
        stack = [(start[0], CHH - 2)]
    reach = 0
    while stack:
        ...
        seen.add((col, row))
        reach = max(reach, col)
        for step in range(1, JUMP_ACROSS + 2):
            for d in (1, -1):
                c = col + d * step
                if c < 0 or c >= cols:
                    continue
                ...
                if r is None:
                    continue
                if step > 1 and cls(c, r - 1) in SOLID_CLASSES:
                    continue
                stack.append((c, r))

    if reach < boss_col + 4:
        raise SystemExit("%s cannot be walked past column %d of %d; the boss "
                         "waits at %d" % (name, reach, cols, boss_col))
    return []
...
def build(write, chr_rom=None):
    ...
    for st in STAGES:
        ...
        log += check_traversal(mapdata, cols, theme.collision,
                               start, boss_col, st.name)
